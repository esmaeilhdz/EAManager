...
use App\Http\Requests\SalePeriod\SalePeriodComboRequest;
...
class SalePeriodController extends Controller
{
    ...
    /**
     * سرویس جزئیات دوره فروش
     * @param SalePeriodComboRequest $request
     * @return JsonResponse
     */
    public function getSalePeriodCombo(SalePeriodComboRequest $request): JsonResponse
    {
        $inputs = $request->validated();
        $this->cleanInput($inputs, array_keys($request->rules()));

        $result = SalePeriodFacade::getSalePeriodCombo($inputs);
        return $this->api_response->response($result['result'], $result['message'], $result['data']);
    }
